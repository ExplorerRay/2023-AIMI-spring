...
def pretext_train_test(root_folder, k, N, epoch_sec):
    all_index = []
    all_index.append('SC4642')
    all_index.append('SC4731')
    all_index.append('SC4092')
    ...
    sample_process(root_folder, k, N, epoch_sec, 'train', all_index)
    print ()
...
def sample_process(root_folder, k, N, epoch_sec, train_test_val, index):
    for i, j in enumerate(index):
        if i % N == k:
            ...
            X = data.get_data()[mne.pick_channels(data.ch_names, ['EEG Fpz-Cz']), :]
            ...
            #     continue
            path = './SLEEP_data/cassette_processed/{}/'.format(train_test_val) + 'cassette-' + j + '.pkl'
            pickle.dump({'X': X, 'y': labels}, open(path, 'wb'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--multiprocess', type=int, default=3, help="How many processes to use")
    ...
    root_folder = './SLEEP_data/sleep-cassette'
