...
class SeqSleepNet_Sleep(object):
    ...
    def __init__(self, config):
        ...
        with tf.device('/gpu:0'), tf.compat.v1.variable_scope("filterbank-layer-eeg"):
            ...
            self.Weeg = tf.Variable(tf.compat.v1.random_normal([self.config.ndim, self.config.nfilter],dtype=tf.float32))
        ...
        if(self.config.nchannel > 1):
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope("filterbank-layer-eog"):
                ...
                # first filter bank layer
                self.Weog = tf.Variable(tf.compat.v1.random_normal([self.config.ndim, self.config.nfilter],dtype=tf.float32))
                # non-negative constraints
        ...
        if(self.config.nchannel > 2):
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope("filterbank-layer-emg"):
                ...
                # first filter bank layer
                self.Wemg = tf.Variable(tf.compat.v1.random_normal([self.config.ndim, self.config.nfilter],dtype=tf.float32))
                # non-negative constraints
